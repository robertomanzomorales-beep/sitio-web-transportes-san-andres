...
export async function POST(request: Request) {
  try {
    ...
    const mailTo = process.env.MAIL_TO || "admsandres@gmail.com";
    const mailFromName =
    ...
    const safe = {
      ...
      message: escapeHtml(message || "Sin información adicional").replaceAll(
        "\n",
        "<br />"
      ),
    };
        ...
        <div style="margin:0;padding:32px;background:#f4f5f7;font-family:Arial,Helvetica,sans-serif;color:#10243a;">
          <div style="max-width:680px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
            <div style="padding:26px 30px;background:#061d35;color:#ffffff;">
              <div style="font-size:12px;font-weight:700;letter-spacing:.12em;color:#ffd400;">TRANSPORTES SAN ANDRÉS</div>
              <h1 style="margin:10px 0 0;font-size:24px;line-height:1.2;">Nueva solicitud de cotización</h1>
            </div>

            <div style="padding:30px;">
              <table style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.6;">
                <tr><td style="padding:9px 0;color:#657382;width:190px;">Nombre</td><td style="padding:9px 0;font-weight:700;">${safe.name}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Empresa / institución</td><td style="padding:9px 0;">${safe.company}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Teléfono</td><td style="padding:9px 0;">${safe.phone}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Correo</td><td style="padding:9px 0;">${safe.email}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Tipo de servicio</td><td style="padding:9px 0;">${safe.service}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Origen</td><td style="padding:9px 0;">${safe.origin}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Destino</td><td style="padding:9px 0;">${safe.destination}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Fecha</td><td style="padding:9px 0;">${safe.date}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Horario</td><td style="padding:9px 0;">${safe.time}</td></tr>
                <tr><td style="padding:9px 0;color:#657382;">Pasajeros</td><td style="padding:9px 0;">${safe.passengers}</td></tr>
              </table>

              <div style="margin-top:24px;padding-top:22px;border-top:1px solid #e5e7eb;">
                <div style="margin-bottom:8px;font-size:12px;font-weight:700;letter-spacing:.08em;color:#e21b16;">INFORMACIÓN ADICIONAL</div>
                <div style="font-size:14px;line-height:1.7;color:#334155;">${safe.message}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    // Si el servidor SMTP restringe correos salientes externos,
    // el correo interno ya habrá sido enviado y este segundo envío puede fallar.
    try {
      await transporter.sendMail({
        from: `"${mailFromName}" <${smtpUser}>`,
        to: email,
          ...
          <div style="margin:0;padding:32px;background:#f4f5f7;font-family:Arial,Helvetica,sans-serif;color:#10243a;">
            <div style="max-width:620px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
              ...
              <div style="padding:32px;">
                <div style="font-size:12px;font-weight:700;letter-spacing:.12em;color:#e21b16;">TRANSPORTES SAN ANDRÉS</div>
                <h1 style="margin:12px 0 18px;font-size:26px;line-height:1.2;color:#061d35;">Recibimos su solicitud</h1>
                ...
                <p style="margin:0;font-size:14px;line-height:1.7;color:#64748b;">
                  Si necesita complementar la información, puede responder directamente este correo o comunicarse al +56 9 2627 3108.
                </p>
              </div>
            </div>
          </div>
      ...
      console.error(
        "La solicitud llegó, pero falló el correo automático al cliente:",
        confirmationError
      );
    }
